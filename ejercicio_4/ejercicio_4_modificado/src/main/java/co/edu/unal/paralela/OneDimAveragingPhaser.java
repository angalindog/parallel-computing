        ...
        for (int ii = 0; ii < tasks; ii++) {
            ...
            threads[ii] = new Thread(() -> {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    ...
                    // Calcular los elementos de la frontera primero
                    if (left <= right) {
                    ...
                    if (left < right) {
                        threadPrivateMyNew[right] = (threadPrivateMyVal[right - 1] +
                                threadPrivateMyVal[right + 1])
                                / 2.0;
                    }

                    // Avisar que las fronteras ya fueron calculadas
                    int phase = phs[i].arrive();

                    // Calcular los elementos interiores mientras tanto
                    for (int j = left + 1; j < right; j++) {
                        threadPrivateMyNew[j] = (threadPrivateMyVal[j - 1] + threadPrivateMyVal[j +
                                1]) / 2.0;
                    }

                    // Esperar a que los vecinos terminen de calcular sus fronteras
                    if (i - 1 >= 0) {
